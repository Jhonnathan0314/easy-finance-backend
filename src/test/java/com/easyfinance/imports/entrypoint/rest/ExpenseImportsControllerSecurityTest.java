...
@SpringJUnitWebConfig(classes = ExpenseImportsControllerSecurityTest.TestConfig.class)
class ExpenseImportsControllerSecurityTest {
    ...
    void importsRequireToken() throws Exception {
        mockMvc.perform(get("/api/v1/accounts/1/imports/expenses/10"))
                .andExpect(status().isUnauthorized())
                .andExpect(jsonPath("$.code").value("UNAUTHENTICATED"));
    }

    @Test
    void templateRequiresToken() throws Exception {
    ...
    void invalidTokenReturnsUnauthorized() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/accounts/1/imports/expenses/10").header("Authorization", "Bearer bad-token"))
                .andExpect(status().isUnauthorized())
    ...
    @EnableWebMvc
    @EnableWebSecurity
    @Import({ExpenseImportsController.class, SecurityConfig.class, JwtAuthenticationFilter.class, RestSecurityExceptionHandler.class})
    static class TestConfig {
        @Bean PreviewExpenseImportPort previewExpenseImportPort() { return mock(PreviewExpenseImportPort.class); }
        @Bean ConfirmExpenseImportPort confirmExpenseImportPort() { return mock(ConfirmExpenseImportPort.class); }
        @Bean GetExpenseImportBatchPort getExpenseImportBatchPort() { return mock(GetExpenseImportBatchPort.class); }
        @Bean GenerateExpenseImportTemplatePort generateExpenseImportTemplatePort() { return mock(GenerateExpenseImportTemplatePort.class); }
