...
@Component
public class ApachePoiPaymentMethodImportTemplateGenerator implements PaymentMethodImportTemplateGeneratorPort {
    ...
    public byte[] generate() {
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream output = new ByteArrayOutputStream()) {
            Sheet paymentMethods = workbook.createSheet(SHEET);
            Sheet values = workbook.createSheet(VALUES_SHEET);

            createValuesSheet(values);
            createNamedRange(workbook);
            createMainSheet(workbook, paymentMethods);
            workbook.setSheetHidden(workbook.getSheetIndex(values), true);

            workbook.write(output);
            return output.toByteArray();
        } catch (IOException ex) {
            throw new IllegalStateException("Payment method import template could not be generated.", ex);
        }
    }
    ...
    private static void createValuesSheet(Sheet sheet) {
        ...
        sheet.createRow(1).createCell(0).setCellValue("Efectivo");
        sheet.createRow(2).createCell(0).setCellValue("CuentaBancaria");
        sheet.createRow(3).createCell(0).setCellValue("TarjetaCredito");
        sheet.createRow(4).createCell(0).setCellValue("TarjetaDebito");
        sheet.createRow(5).createCell(0).setCellValue("BilleteraDigital");
        sheet.createRow(6).createCell(0).setCellValue("Otro");
    ...
    private static void createNamedRange(Workbook workbook) {
        ...
        range.setNameName(TYPES_RANGE);
        range.setRefersToFormula("'" + VALUES_SHEET + "'!$A$2:$A$7");
    }

    private static void createMainSheet(Workbook workbook, Sheet sheet) {
    ...
    private static void addTypeValidation(Sheet sheet) {
        ...
        DataValidationConstraint constraint = helper.createFormulaListConstraint(TYPES_RANGE);
        DataValidation validation = helper.createValidation(constraint, new CellRangeAddressList(1, MAX_DATA_ROWS, 1, 1));
        validation.setShowErrorBox(true);
        validation.createErrorBox("Tipo invalido", "Use un tipo permitido de la lista.");
